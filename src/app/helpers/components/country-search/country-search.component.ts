...
@Component({
    selector: "skrambler-country-search",
    templateUrl: "./country-search.component.html"
})
export class CountrySearchComponent extends SkramblerFormBaseComponent implements OnInit {
    ...
    /**
     * search event triggered on any change in the input field
    ...
    ngOnInit() {
        ...
        if (this.isRequiredField) {
            let requiredValidation: ValidatorFn[] = [Validators.required];
    ...
    optionSelected(event) {
        ...
        this.formGroup.controls["search_box"].setValue(event.option.value.country_name + " - +" + event.option.value.country_phone_code);
